...
- This agent fixed the next two runtime failures found live on Codespaces:
  ...
  2. **`Blocked import of regex from current working directory` (Root cause UPDATED 2026-08-02, second pass):** NOT Ubuntu-patched CPython as first thought — it's **nltk 3.10.x's new `inisec.py` import-hijack guard** (official PyPI wheel, added 2026-07-31). Its naive `spec.origin.relative_to(cwd)` check treats an **in-project venv's own site-packages** as "CWD", so `nltk -> text.py -> import regex` from `.venv/` gets blocked whenever cwd=repo root. `PYTHONSAFEPATH`/`PYTHONPATH` spawn env kept as defense-in-depth for true CWD shadowing, but the complete fix = **`NLTK_DISABLE_IMPORT_SECURITY=1`** (official kill switch) in `run.ts` spawn + setup/doctor scripts, **plus pin `nltk>=3.9.0,<3.10`** (3.9.4 verified clean, no inisec). Revisit pin after upstream patch — this likely hits every in-project-venv user of nltk 3.10.x.
